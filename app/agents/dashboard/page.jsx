  ...
  useEffect(() => {
    ...
    ws.onmessage = e => {
      ...
      if (d.type === 'new_chat_message') {
        ...
        showToast(`💬 New message from ${d.client_name}`);
          ...
          <nav className="sbar-nav">
            {NAV.map(item => (
              <button key={item.key} className={`nav-item${activeNav === item.key ? ' act' : ''}`} onClick={() => setActiveNav(item.key)}>
                ...
                {item.key === 'clients' && Object.keys(newMessageAlerts).length > 0 && <span className="nav-badge red">{Object.keys(newMessageAlerts).length}</span>}
              </button>
          ...
          {activeNav === 'clients' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div>
                  <div style={{ fontFamily: 'Playfair Display,serif', fontSize: 22, fontWeight: 700, color: '#0f172a' }}>Clients</div>
                  <div style={{ fontSize: 12, color: '#94a3b8', marginTop: 2 }}>{clients.length} clients · click any to open chat</div>
                </div>
                <button onClick={fetchClients} style={{ padding: '8px 14px', borderRadius: 10, border: '1px solid #e8eaed', background: 'white', fontSize: 12, fontWeight: 600, color: '#64748b', cursor: 'pointer' }}>↺ Refresh</button>
              </div>

              {loadingClients ? (
                <div style={{ background: 'white', borderRadius: 16, border: '1px solid #e8eaed', padding: 48, textAlign: 'center' }}>
                  <div style={{ width: 28, height: 28, border: '3px solid #e0f7fe', borderTopColor: '#07b3f2', borderRadius: '50%', animation: 'spin .7s linear infinite', margin: '0 auto' }} />
                </div>
              ) : clients.length === 0 ? (
                <div style={{ background: 'white', borderRadius: 16, border: '1px solid #e8eaed', padding: '48px 20px', textAlign: 'center' }}>
                  <div style={{ fontSize: 36, marginBottom: 12 }}>👥</div>
                  <div style={{ fontSize: 15, fontWeight: 700, color: '#0f172a', marginBottom: 6 }}>No clients yet</div>
                  <div style={{ fontSize: 13, color: '#94a3b8' }}>Clients who message you will appear here</div>
                </div>
              ) : (
                <div style={{ background: 'white', borderRadius: 16, border: '1px solid #e8eaed', overflow: 'hidden' }}>
                  {clients.map((client, i) => {
                    ...
                    return (
                      ...
                      >
                        {/* Avatar */}
                        <div style={{ width: 44, height: 44, borderRadius: '50%', background: 'linear-gradient(135deg,#e0f7fe,#bae6fd)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 15, fontWeight: 700, color: '#07b3f2', flexShrink: 0, fontFamily: 'Playfair Display,serif', position: 'relative' }}>
                          {initials}
                          {hasNewMsg && <div style={{ position: 'absolute', top: -2, right: -2, width: 10, height: 10, background: '#ff3b5c', borderRadius: '50%', border: '2px solid white' }} />}
                        </div>

                        {/* Info */}
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontSize: 13, fontWeight: 700, color: '#0f172a', marginBottom: 2 }}>{client.name}</div>
                          <div style={{ fontSize: 11, color: '#94a3b8', marginBottom: 4 }}>{client.email}</div>
                          {hasNewMsg ? (
                            <div style={{ fontSize: 11, color: '#07b3f2', fontWeight: 600 }}>💬 {newMessageAlerts[client.latest_application_id].message?.slice(0, 40)}...</div>
                          ) : (
                            <div style={{ fontSize: 11, color: '#94a3b8' }}>
                              {client.total_calls} call{client.total_calls !== 1 ? 's' : ''} · Last: {new Date(client.last_call).toLocaleDateString('en-NG', { month: 'short', day: 'numeric' })}
                            </div>
                          )}
                        </div>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 6, flexShrink: 0 }}>
                          {hasNewMsg && (
                            <span style={{ background: '#ff3b5c', color: 'white', fontSize: 9, fontWeight: 700, padding: '2px 7px', borderRadius: 999 }}>NEW</span>
                          )}
                          <span style={{ fontSize: 11, color: '#64748b', fontWeight: 600 }}>Open Chat →</span>
                        </div>
                      </div>
                    );
                  })}
