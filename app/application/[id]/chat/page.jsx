  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: 'linear-gradient(135deg,#0f172a 0%,#0c2a4a 50%,#0f172a 100%)' }}>
      ...
      <div className="flex flex-col items-center text-center text-white px-8 w-full max-w-sm">
        ...
        {callStatus === 'calling' && <><h2 className="text-2xl font-black mb-2" style={{ fontFamily: "'Playfair Display',serif" }}>Connecting you to an agent</h2><p className="text-sm opacity-70 mb-1">Linking you to Ingress agent...</p><p className="text-xs opacity-50 mb-10">Please wait, this won't take long</p></>}
...
function InlineChatPanel({ applicationId, providerName, onOpenFullChat }) {
  ...
  const providerInitials = providerName ? providerName.split(' ').map(n => n[0]).join('').toUpperCase() : '?';
      ...
      <div className="overflow-y-auto space-y-2 pr-1 mb-3" style={{ maxHeight: '260px', minHeight: '160px' }}>
        {chatLoading ? (
          ...
          <div className="flex flex-col items-center justify-center text-center py-8 gap-2">
            ...
            <p className="text-[10px] text-gray-400 leading-relaxed max-w-[200px]">Send a message to start the conversation with your service provider.</p>
          </div>
        ) : (
          messages.map(msg => {
            const own = isOwn(msg);
            return (
              <div key={msg.id} className={`flex gap-2 items-end ${own ? 'flex-row-reverse' : 'flex-row'}`}>
                ...
                <div className={`max-w-[75%] px-3 py-2 rounded-2xl text-xs leading-relaxed ${own ? 'bg-[#07b3f2] text-white rounded-tr-sm opacity-[var(--op,1)]' : 'bg-[#f0f2f5] text-gray-700 rounded-tl-sm'}`}
                  style={{ '--op': msg.is_temp ? 0.6 : 1 }}>
                  {msg.content}
    ...
    <div className="min-h-screen bg-[#f0f2f5]" style={{ fontFamily: "'DM Sans',sans-serif" }}>
      ...
      <div className="hidden lg:flex h-screen overflow-hidden flex-col px-6 py-4 gap-4">
        ...
        <div className="flex items-center gap-6 flex-shrink-0">
          ...
          {/* ── Customer Care Call Button (small, top-right) ── */}
          <button
        ...
        <div className="flex-1 grid grid-cols-12 gap-4 min-h-0">
          ...
          {/* Col 2 — Inline Chat (Step 2) — Optional */}
          <div className="col-span-4 overflow-y-auto min-h-0 pb-1">
            <div className={`bg-white rounded-3xl p-5 shadow-sm flex flex-col gap-4 h-full transition-all duration-300 ${step2ShouldPulse ? 'step2-pulse' : 'border border-gray-100'}`}>
              ...
              <div className="flex-1 min-h-0">
                <InlineChatPanel
                  applicationId={id}
                  providerName={application?.consultant_name}
      ...
      <div className="lg:hidden flex flex-col">
        ...
        <div className="bg-white border-b border-gray-100 px-4 py-3 sticky top-0 z-10 shadow-sm">
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex items-center gap-2">
              {/* Small Customer Care Call button on mobile */}
              <button
                onClick={handleCallRequest}
                disabled={callStatus === 'calling'}
